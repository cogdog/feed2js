...
/*  Feed2JS : RSS feed to JavaScript src file
   ...
   Feed2JS makes use of a modified version of rss-php from
...
// Disassemble src into component parts
$parsed_url_src = parse_url($src);
// Re-assemble src, currently only works with absolute URLs
...
if (isset($restrict_url) && substr($src_host, strlen($src_host)-strlen($restrict_url)) != $restrict_url) {
	...
	// no feed found, return error statement
	if  (!$rss) {
		...
		if ($chan == 'y')
        {
			...
			$str.= "document.write('<p class=\"rss-title\"><a class=\"rss-title\" href=\"" .
                   trim(check_link($rss['link'])) . '"' .
                   $target_window . ">" . addslashes(strip_returns(check_array($rss['title']))) .
                   "</a><br /><span class=\"rss-item\">" .
                   addslashes(strip_returns(strip_tags(isset($rss['description']) ? check_array($rss['description']) : check_array($rss['subtitle'])))) .
                   "</span></p>');\n";
        ...
        elseif ($chan == 'titlelinkno')
        {
			...
			$str.= "document.write('<p class=\"rss-title\">" .
                   addslashes(strip_returns(check_array($rss['title']))) . "</p>');\n";
        ...
        elseif ($chan == 'linkno')
        {
			...
			$str.= "document.write('<p class=\"rss-title\">" .
                   addslashes(strip_returns(check_array($rss['title']))) .
                   "<br /><span class=\"rss-item\">" .
                   addslashes(strip_returns(strip_tags(isset($rss['description']) ? check_array($rss['description']) : check_array($rss['subtitle'])))) . "</span></p>');\n";
		}
		...
		foreach ( $all_items as $item ) {
			...
			if ($desc < 0) {
				...
				// format item title
				$my_title = addslashes(strip_returns(check_array($item['title'])));
			...
			if ($desc)
            {
				if (isset($item['atom_content']))
                {
					...
					$my_blurb = ($item['atom_content'] == "A") ? check_array($item['description']) : html_entity_decode ( check_array($item['atom_content']), ENT_NOQUOTES, 'UTF-8');
				}
                elseif (isset($item['summary']))
                {
					// Atom/encocded content support (thanks David Carter-Tod)
					$my_blurb =   html_entity_decode ( check_array($item['summary']), ENT_NOQUOTES, 'UTF-8');
				}
                elseif (isset($item['description']))
                {
					// Atom/encocded content support (thanks David Carter-Tod)
					$my_blurb =   html_entity_decode ( check_array($item['description']), ENT_NOQUOTES, 'UTF-8');
				}
                elseif (isset($item['content']))
                {
					// Atom/encocded content support (thanks David Carter-Tod)
					$my_blurb =   html_entity_decode ( check_array($item['content']), ENT_NOQUOTES, 'UTF-8');
				}
                else
                {   
					$my_blurb = "";
...
function check_array($field)
{
    if (is_array($field))
    {
        if (false && !isset($field['value']))
        {
            print "<pre>\n";
            print_r($field);
            print "</pre>\n";
